...
def get_personal_data():
    ...
    orcid = "0009-0009-9691-7852"

    bio_text = f"""
                <p>
                    I am a Ph.D. Candidate at the <a href="http://cssp.lab.nycu.edu.tw/" target="_blank">Chaotic Systems and Signal Processing Laboratory (CSSP Lab.)</a> at National Yang Ming Chiao Tung University (NYCU), Taiwan. 
                    My research focuses on  developing cognitively-inspired AI systems with memory interpretability and consolidation capabilities for autonomous driving applications. 
                    Representative papers are <span style="background-color:#ffffd0">highlighted</span> below.
                </p>
                <p>
                ...
                <!--  <a href="https://sshouhua.github.io/assets/pdf/cwsun_cv.pdf" target="_blank" style="margin-right: 15px"><i class="fa fa-address-card fa-lg"></i> CV</a> -->
                    ...
                    <a href="https://orcid.org/{orcid}" target="_blank" style="margin-right: 15px"><i class="fab fa-orcid fa-lg"></i> ORCID</a>
                </p>
    """
    footer = """
...
def get_author_dict():
    return {
        'Chih-Wei Tseng': 'https://scholar.google.com/citations?user=ybjfgNEAAAAJ&hl=en',
        'Bing-Fei Wu': 'https://scholar.google.com/citations?user=7-23WmIAAAAJ&hl=en',
        'Chang-Hsin Lee': 'https://lee-910530.github.io/',
        'Yu-Chen Lin': 'https://scholar.google.com/citations?user=tI26CY8AAAAJ&hl=en',
        'Tsung-Chih Lin': 'https://fcuece.fcu.edu.tw/en/teachers-detail/?id=T77045&unit_id=CE09',
        'Majid Zirkohi': 'https://scholar.google.com/citations?user=8RJj4fUAAAAJ&hl=en',
        }


def generate_person_html(persons, connection=", ", make_bold=True, make_bold_name='Chien-Wen Sun', 
    ...
    for idx, p in enumerate(persons):
        ...
        if idx == corresponding_authors:
            string_part_i += "*"
        if make_bold and string_part_i == make_bold_name:
            string_part_i = f'<span style="font-weight: bold";>{make_bold_name}</span>'
        if idx < equal_contributors:
            string_part_i += "<sup>&dagger;</sup>"
        if p != persons[-1]:
...
def get_paper_entry(entry_key, entry):
    ...
    if 'equal_contribution' in entry.fields.keys():
        s += f"""{generate_person_html(entry.persons['author'], equal_contribution=int(entry.fields['equal_contribution']), corresponding=int(entry.fields['corresponding']))} <br>"""
    else: 
        s += f"""{generate_person_html(entry.persons['author'], corresponding=int(entry.fields['corresponding']))} <br>"""
